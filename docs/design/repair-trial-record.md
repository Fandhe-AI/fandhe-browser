...
- **R-3 の注入**:
  ...
  - **引き渡し手順（差分の追跡不可能化。改修担当が注入ブランチを checkout しても注入箇所を diff・履歴から辿れないようにする）**:
    1. 注入担当は、注入後の作業ツリー全体を親を持たない単一のオーファンコミット（`git checkout --orphan` 等でベースコミットとの親子関係を切った初期コミット 1 件のみ）として `trial/task-2-r3` ブランチに作成する。ベースブランチ（`main` 等）へのマージベースを持たせない（`git merge-base` が解決不能な独立履歴にする）ことで、改修担当が `git diff <ベース>..HEAD`・`git log --all`・`git blame`・`git reflog` のいずれを実行しても、注入前の状態や注入差分を復元できないようにする
    2. 改修担当に渡すのは、このオーファンコミット 1 件だけを含むブランチ（または、そのコミット時点のスナップショットをアーカイブ化した tarball / パッチではない完全ソース一式）であり、注入担当のローカル作業履歴（複数コミット・stash・reflog）を含む worktree や `.git` を渡さない
    3. 改修担当の作業環境（worktree）はこのオーファンブランチから新規に checkout したものに限定し、注入担当の元 worktree の `.git`（reflog・他ブランチ参照を含む）を共有・引き継がない
    4. 改修担当には、失敗するテストの実行結果（`cargo test` の失敗ログ）とオーファンコミット 1 件のブランチのみを渡し、注入担当側で保持する「注入前後の diff」「注入方法のメモ」は一切共有しない
    5. TASK-2.2 実施前に、上記手順で改修担当が `git log`（1 件のみ表示されること）・`git diff`（比較対象がなく差分を計算できないこと）を確認し、注入箇所が履歴から特定不能な状態であることを注入担当が検証してから改修担当へ引き渡す
