...
| R-3 | バグ修正・注入バグ（T-3 相当） | core / `dom.rs` | 試行用ブランチで、実装済みの公開 API 1 件に軽微な不具合（オフバイワン相当）を注入し、改修担当は既存テストの失敗だけを手がかりに原因を特定して修正する。対象箇所・具体的な注入方法は注入担当のみが把握し、リポジトリには記載しない | 約 0.5h |
...
- **R-3**: `dom.rs` 内の対象メソッド 1 件のみ。注入自体は 1〜2 行の改変、修正も同程度
...
### R-3: `dom.rs` への軽微な不具合注入

- 対象は `dom.rs` 内の実装済み公開 API 1 件。既存ロジックに、意図的な軽微な不具合（オフバイワン相当）を注入し、改修担当は失敗するテストの症状だけを手がかりに原因を特定・修正する
- **注入箇所・具体的な注入方法・影響するテスト名は本リポジトリ（コード・ドキュメント・git 履歴）のいずれにも記載しない**。実施要領が定義するとおり、改修担当が原因特定を伴わずに修正できてしまうと単独修復の実証にならないため、詳細は注入担当がリポジトリの外（セッション内メモ等、改修担当がブランチを checkout しても到達できない場所）でのみ保持する
- 注入前に改修担当以外の担当（別 Agent または人間）が、全ゲート（fmt/clippy/test）が通過していることを確認してから注入する（TASK-2.2 の前提条件）
- 注入により既存テストが失敗すること、修正後は専用テストを追加しなくても全ゲートを通過することを、注入担当がリポジトリ外で確認済み
- 試行（TASK-2.2）終了後、注入担当が保持していた詳細のうち「試行結果」章に必要な結果（試行回数・ゲート結果・変更行数等）のみをこの記録に追記する。注入箇所・手法そのものは追記後も本リポジトリに記載しない
...
- **R-3 の注入**:
  ...
  - 注入箇所・具体的な修正方法はリポジトリ・git 履歴の外（注入担当のセッション内メモ等、改修担当がブランチを checkout しても到達できない場所）でのみ保持し、注入担当以外（改修担当を含む）には渡さない
  - 注入ブランチは main に取り込まない
  - TASK-6（#330〜#333、`tests/break_detection.rs`）の「検出実証」とは目的が異なる「修復実証」であり、TASK-6 の成果物を作らない・流用しない
  - **引き渡し手順（差分の追跡不可能化。改修担当が注入後のソースを受け取っても、ベースコミットや `main` の refs と比較して注入箇所を diff・履歴から辿れないようにする）**:
    1. オーファンコミット（`git checkout --orphan` 等で親子関係を切った単一コミット）だけでは不十分である。改修担当に渡す worktree が元リポジトリから作られる限り、`.git` は `main` を含む既存の refs・オブジェクトを共有しており、改修担当が `git fetch`・`git diff main HEAD`・`git log --all` を実行すれば `main`（または `origin/main`）を直接参照でき、注入前後の差分がそのまま特定できてしまう。ブランチの履歴を切っただけでは隔離にならない
    2. 注入担当は、注入後の作業ツリーのファイル一式を、元リポジトリの `.git` を含めずにコピー・アーカイブ化する（例: `git worktree` を使わず `git archive` 等でソースのみを抽出する、または新しいディレクトリへファイルをコピーして `.git` を含めない）。改修担当に渡すのは、このソース一式（または `.git` を持たない状態から改修担当自身に `git init` させる形）のみとする
    3. 改修担当の作業環境は、元リポジトリの clone・worktree・fetch 元のいずれでもない独立した環境（`.git` が存在しないか、存在してもベースコミット・`main`／`origin/main` 等の refs・オブジェクトを一切含まない新規リポジトリ）に限定する。改修担当のネットワークアクセス（`git fetch`・`git clone` 等で元リポジトリの `origin` へ到達できる経路）も遮断し、`main` の refs へ後から取得できないようにする
    4. 改修担当には、失敗するテストの実行結果（`cargo test` の失敗ログ）と上記の独立環境のみを渡し、注入担当側で保持する「注入前後の diff」「注入方法のメモ」「ベースコミットの sha」は一切共有しない
    5. TASK-2.2 実施前に、注入担当が改修担当の環境で `git log`（履歴が無い、またはオーファンコミット 1 件のみで `main` へ到達不能なこと）・`git diff main HEAD` 相当の操作（`main` を解決できず失敗すること）・`git remote -v`（元リポジトリの `origin` へ到達できないこと）を確認し、ベースコミットや `main` の refs にアクセスできない独立した環境であることを検証してから改修担当へ引き渡す
