...
//! 今後、以下のタスクで段階的に実装する：
//!
//! - TASK-11: 役割ベース DOM 簡約表現の中核実装（本 crate の中心機能）
//! - TASK-19（AISNAP-6・AISNAP-7）: `/ai/*` ルータ向けスナップショット API
//! - TASK-92 以降（PLUG-2）: プラグインレジストリ（プロセス分離・stdio 経由の
//!   外部プラグイン呼び出し。動的ライブラリの実行時ロードは行わない方針。
//!   security.md 参照）
