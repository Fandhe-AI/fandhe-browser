...
//! `html5ever = "=0.40.1"`（Issue #35 承認済み・TASK-24.4・#38）のみを持ち、
//! HTML パーサーとして単体で使う（`scraper` 不採用の経緯は `parse` モジュールの
//! doc コメントを参照）。他の依存追加は該当タスクで dependency-policy.md の
//! ユーザー承認制に従って行う。`fandhe-browser-ai`・`fandhe-browser-cdp` 等の
...
//! （[`error::Error`] / [`error::Result`]）を追加した。`parse`（TASK-24.4・#38）は
//! html5ever の `TreeSink` を自作実装し、HTML 文字列・バイト列から `dom::Document`
//! （arena）を構築する本実装を持つ。`dom`（TASK-24.5・#39）は arena の型定義
//! （[`dom::Document`]・[`dom::Node`] 等）のみを持ち、走査 API はまだ実装していない
//! （REPAIR-3: 実装済みを装わない）。`fetch` は TASK-24.2（#36）、`query` は
//! TASK-24.7（#41）でそれぞれ本実装を行う。
