...
# 呼ぶため自動的にこの検証を含む。
.PHONY: ci
ci: lint-docs check-workspace-manifest fmt-check lint lint-rendering check-render-isolation check-publish-private test test-rendering deny check-deny-license-reject ## ローカルゲート（.claude/rules/ci.md）と同等のチェックを一括実行する
